...
- **Read input:** `input.F90` ~lines 2329–2336 -- Checks whether `nfix_option_input.csv` exists; if so, CE-QUAL-W2 reads `CRIT_TIN(JA)` for each algal group. If the file is absent, `CRIT_TIN` defaults to 0.0 for all groups and the feature is inactive.
